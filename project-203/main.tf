...
provider "github" {
  # Configuration options
  token = "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"
}

resource "github_repository" "myrepo" {
  name = "TF-bookstore-repo"
  auto_init = true
  visibility = "private"
}
...
resource "aws_instance" "tf-docker-ec2" {
  ...
  key_name      = "aws-key"
  vpc_security_group_ids = [aws_security_group.tf-docker-sec-gr-203.id]
  tags = {
  ...
  user_data = <<-EOF
          ...
          TOKEN="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"
          FOLDER="https://$TOKEN@raw.githubusercontent.com/metindrk/TF-bookstore-repo/main/"
          ...
          docker build -t mtndrk/bookstoreapi:Latest .
          docker-compose up -d
          EOF
